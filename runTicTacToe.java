...
public class runTicTacToe {
	...
	private aiTicTacToe ai1; // player 1 with X
	private aiTicTacToe ai2; // player 2 with O
	...
	public int[] run()
	{
		...
		//int turn = 1; //player1 always go first
		int winner = -1;
		int num = 0;
	...
	public static void main(String[] args) {		
		...
		System.out.println("one: "+one+"; two: "+two+"; draw: "+draw+"; num of moves: "+decnum);
		long endTime = System.nanoTime();

		// get difference of two nanoTime values
		long timeElapsed = endTime - startTime;

		System.out.println("Execution time in milliseconds : " + (timeElapsed / decnum) / 1000000.0);

		/*// Calculate the number of strongest points
		int[][][] boardarr = new int[4][4][4];
		...
		for(int i = 0; i < rttt.winningLines.size(); i++) {
			for(int j = 0; j < rttt.winningLines.get(i).size(); j++) {
				boardarr[rttt.winningLines.get(i).get(j).x][rttt.winningLines.get(i).get(j).y][rttt.winningLines.get(i).get(j).z]++;
			}
		}
		...
		for(int i=0;i<4;i++) {
			...
			for(int j=0;j<4;j++) {
				...
				for(int k=0;k<4;k++)
					{
						System.out.print(boardarr[i][j][k]);
					}		
				System.out.print("]\n");			
			}
		}*/
